...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    {
        if (GetMonData(&mons[slotId], i + MON_DATA_MOVE1) == MOVE_NONE) {
            break;
        }
...
void ItemUseCB_Mint(u8 taskId, TaskFunc task)
{
    ...
    tState = 0;
    tMonId = gPartyMenu.slotId;
    tOldNature = GetMonData(&gPlayerParty[tMonId], MON_DATA_PERSONALITY);
    tNewNature = ItemId_GetSecondaryId(gSpecialVar_ItemId) - 38;
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUseEffect)
    {
        ...
        {
            if (gSpecialVar_ItemId != ITEM_INFINITY_CANDY) {
                RemoveBagItem(gSpecialVar_ItemId, 1);
            }
            FreePartyPointers();
